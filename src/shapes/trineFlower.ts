...
export class TrineFlower extends baseShape {
    private hue: number
    ...
    draw() {
        if (this.ctx) {
            ...
            this.ctx.lineTo(posX -5* this.seed+1, posY -20* this.seed+1);
            this.ctx.lineTo(posX -20* this.seed+1 , posY -5* this.seed +1);
